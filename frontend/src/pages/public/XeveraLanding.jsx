...
const STATS = [
  { key: 'total', label: 'Reports Submitted', icon: 'file', color: '#1258E8' },
...
const BENEFITS = [
  {
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="m8 12 3 3 5-5"/></svg>
    ),
  ...
  return (
    <div className="overflow-x-hidden w-full max-w-[100vw]">
      ...
      <section className="relative w-full overflow-hidden">
        ...
        >
          {/* Dark blue gradient overlay — LEFT side only */}
          <div
            className="absolute inset-0 pointer-events-none hidden md:block"
            style={{
              background:
                'linear-gradient(90deg, rgba(5,22,54,0.92) 0%, rgba(5,22,54,0.82) 28%, rgba(5,22,54,0.50) 52%, rgba(5,22,54,0.10) 72%, transparent 100%)',
            }}
            aria-hidden="true"
          />
          {/* Mobile overlay */}
          <div
            ...
            style={{
              background:
                'linear-gradient(90deg, rgba(5,22,54,0.92) 0%, rgba(5,22,54,0.72) 45%, rgba(5,22,54,0.30) 100%)',
            }}
            aria-hidden="true"
          />
          ...
          <div className="relative z-10 w-full pl-6 pr-6 sm:pl-10 sm:pr-6 md:pl-16 md:pr-8 lg:pl-20 lg:pr-12 max-w-[1500px] mx-auto flex flex-col justify-center">
            {/* Badge */}
            <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-white/95 backdrop-blur-sm self-start px-3 py-1.5 sm:px-3.5 sm:py-1.5 text-[10px] sm:text-[11px] font-bold tracking-[0.14em] sm:tracking-[0.16em] uppercase text-[#1264f5] shadow-[0_4px_12px_rgba(8,28,72,0.18)]">
            ...
            <div className="mt-7 sm:mt-6 flex flex-col sm:flex-row sm:items-stretch gap-3 sm:gap-3 w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 min-h-[50px] sm:min-h-[48px] px-7 sm:px-8 rounded-[12px] bg-[#1264f5] border border-[#1264f5] text-white text-[15px] font-extrabold shadow-[0_8px_20px_rgba(18,100,245,0.35)] hover:bg-[#0d52d6] hover:-translate-y-[1px] active:translate-y-0 transition-all duration-200 cursor-pointer"
              >
                Login
              </button>
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 min-h-[50px] sm:min-h-[48px] px-7 sm:px-8 rounded-[12px] bg-white border border-white text-[#1264f5] text-[15px] font-extrabold shadow-[0_6px_16px_rgba(8,28,72,0.18)] hover:bg-[#F5F8FD] hover:-translate-y-[1px] active:translate-y-0 transition-all duration-200 cursor-pointer"
              >
                Create Account
              </button>
            </div>
            ...
            <div className="mt-8 sm:mt-8 flex flex-wrap items-center gap-4 sm:gap-6">
              {BENEFITS.map((b) => (
                <div key={b.label} className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-[10px] bg-white/15 backdrop-blur-sm flex items-center justify-center text-white border border-white/20">
                    {b.icon}
                  </div>
      ...
      {/* ===== IMPACT SECTION ===== */}
      <section className="max-w-[1400px] mx-auto px-4 sm:px-8 pt-10 sm:pt-12 lg:pt-14 pb-12 sm:pb-16 lg:pb-20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
          {STATS.map((s) => {
            ...
            return (
              <div
                key={s.key}
                className="bg-white rounded-[16px] sm:rounded-[20px] border border-[#E5E7EB] p-5 sm:p-6 hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(10,26,69,0.10)] transition-all duration-300 min-w-0"
              >
                <div className="w-11 h-11 rounded-[12px] flex items-center justify-center text-white shadow-md flex-shrink-0"
                  ...
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {s.icon === 'check' && <><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></>}
                    {s.icon === 'clock' && <><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></>}
                    {s.icon === 'thumbsup' && <><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3H14z"/><path d="M7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"/></>}
                  </svg>
                </div>
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-8 pt-4 sm:pt-6 pb-12 sm:pb-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
          {[
            ...
            {
              icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 11h-6"/><path d="M19 8v6"/></svg>,
              title: 'Community Focused',
              desc: 'Better service for a better Xevera community.',
              color: '#1EA85B',
            },
            {
              icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m13 2-2 7 4 2-8 11 2-7-4-2 8-11Z"/></svg>,
              title: 'Fast & Easy',
