...
export default function ForcePasswordChangePage({ userName, onDone, onLogout }) {
  ...
  const reqsOk = useMemo(() => PW_REQS.map((r) => r.test(next)), [next]);
  ...
  // First-login: verify via purpose=password_change_first_login. No
  // current password - backend allows empty current_password for this
  // purpose; resend-otp / verify-otp already include it in allowlists.
  useEffect(() => {
  ...
  async function applyChange() {
    ...
    try {
      await apiFetch('profile/password_change_complete.php', {
        method: 'POST',
        body: { new_password: next, confirm_password: confirm },
      });
      setStep('done');
    } catch (err) {
  ...
  // Step 1 -> Step 2 -> Step 3 (handed back to the page via the same
  // OtpVerificationPage used by login / registration / password-reset,
  // with a custom purpose so the correct record is matched.)
  if (step === 'otp') {
    return (
      <OtpVerificationPage
        email={user?.email}
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_300px] gap-4 items-start" style={{ maxWidth: 860 }}>
        ...
        <aside className="bg-white rounded-[18px] border border-[#E5E7EB] p-6">
          ...
          <ul className="mt-3 space-y-2">
            {PW_REQS.map((r, i) => (
              <li key={r.label} className={`flex items-center gap-2 text-[12px] ${reqsOk[i] ? 'text-[#128A4C]' : 'text-[#4C638B]'}`}>
            ...
            <li className={`flex items-center gap-2 text-[12px] ${matches ? 'text-[#128A4C]' : 'text-[#4C638B]'}`}>
              <span className={`font-black text-[14px] ${matches ? '' : 'opacity-35'}`}>✓</span>Passwords match
            </li>
          </ul>
        </aside>
      </div>
